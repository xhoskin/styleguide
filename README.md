...
### 2.x. Имя вложенного файла должно начинаться с нижнего подчеривания.
...
### 2.x. Все стили блока должны быть в отдельном файле с названием блока.
...
нем будут описаны причины, почему нам не подходит определенные правила
разделения на папки.
...
### 2.x. Все переменные в одном файле
Облегчаем этим поиск нужного файла. В дальнейшем [обсудим](TODO:ссылка-на-обсуждение),
...
// файл input.css
.input { color: $button-color; } // устанешь искать, где же опредение переменной $button-color
```
Хорошо
```
// файл _variables.css
...
### 2.x. Нет миксинов и классов для border и т.п.
// TODO: 
